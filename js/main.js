...
function initShowcase(){
  ...
  showcaseTrack.innerHTML=SHOWCASE_IMAGES.map((p,i)=>{
    ...
    const imgEl= i===0
      ? `<picture><source type="image/avif" srcset="${p.avif}" media="(min-width:769px)"><source type="image/webp" srcset="${p.thumb} 400w, ${p.mid} 540w, ${p.url} 900w" sizes="(max-width:900px) 60vw, 21vw"><img src="${p.thumb}" alt="${p.alt}" loading="eager" fetchpriority="high" decoding="sync"${pos}></picture>`
      : `<picture><source type="image/avif" data-srcset="${p.avif}" media="(min-width:769px)"><source type="image/webp" data-srcset="${p.thumb} 400w, ${p.mid} 540w, ${p.url} 900w" sizes="(max-width:900px) 60vw, 21vw"><img data-src="${p.thumb}" src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg'/%3E" alt="${p.alt}" loading="eager" decoding="async"${pos}></picture>`;
